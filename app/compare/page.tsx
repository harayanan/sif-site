...
export default function ComparePage() {
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
        ...
        <table className="w-full text-sm border-collapse">
              ...
              {comparisonData.headers.map((h, i) => (
                <th
                  ...
                  className={`px-6 py-4 text-left text-xs uppercase tracking-wider font-semibold ${
                    i === 1
                      ? "bg-brand-red/10 text-brand-red border-b-2 border-brand-red/30"
                      : "bg-brand-navy-light text-gray-400 border-b border-blue-subtle"
                  } ${i === 0 ? "rounded-tl-xl" : ""} ${
          ...
          <tbody>
            {comparisonData.rows.map((row, ri) => (
              <tr key={ri} className="border-b border-blue-subtle hover:bg-brand-navy-light/50 transition-colors">
                {row.map((cell, ci) => (
                  <td
                    key={ci}
                    className={`px-6 py-4 font-light ${
                      ci === 0
                        ...
                        : ci === 1
                        ? "font-semibold text-white bg-brand-red/[0.03]"
                        : "text-gray-500"
                    }`}
                  >
                    {cell}
      ...
      {/* Key takeaway */}
      <div className="bg-brand-navy-light border border-blue-subtle rounded-2xl p-10 sm:p-12">
        <h3 className="text-sm font-semibold text-brand-gold uppercase tracking-[0.2em] mb-6">
          Why Choose SIF?
        </h3>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-8">
          <div>
            <p className="text-3xl font-display font-bold text-brand-red mb-2">₹10L</p>
            <p className="text-sm text-gray-500 font-light">
              Accessible entry vs ₹50L for PMS and ₹1Cr for AIFs
            </p>
          </div>
          <div>
            <p className="text-3xl font-display font-bold text-brand-red mb-2">MF Tax</p>
            <p className="text-sm text-gray-500 font-light">
              Same tax-efficient structure as mutual funds
            </p>
          </div>
          <div>
            <p className="text-3xl font-display font-bold text-brand-red mb-2">Daily NAV</p>
            <p className="text-sm text-gray-500 font-light">
